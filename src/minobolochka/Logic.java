...
public class Logic {
    ...
    Point[] points = new Point[10]; // fixme
        ...
        do {
            for (int j = 1; j < Point.id; j++) {
                if (vect(points2[k], points[min], points2[k], points[j]) < 0
                        ...
                        && (dist2(points2[k], points[min]) < dist2(points2[k], points[j])))) min = j;
            }
           if (k<10) k++;
            points2[k] = points[min];
            min = 0;
             System.out.println(points2[k].x + "<x>" + points2[0].x);
             System.out.println(points2[k].y + "<y>" + points2[0].y);
        } while (v(k));
        System.out.println(points2[k].x + "<x>" + points2[0].x);
        System.out.println(points2[k].y + "<y>" + points2[0].y);
        return points2;
